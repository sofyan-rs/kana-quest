<script lang="ts">
	import { hiraganaList } from '$lib/data/hiragana';
	...
	function startGame() {
		...
		// Pick new answer
		answer = hiraganaList[Math.floor(Math.random() * hiraganaList.length)];

		// Generate options
		options = hiraganaList.filter((item) => item !== answer);
	...
	function nextQuestion() {
		...
		let available = hiraganaList.filter((item) => !prevAnswers.includes(item));

		// If all items used, reset
		if (available.length === 0) {
			prevAnswers = [];
			available = [...hiraganaList];
		}
		// Pick new answer
